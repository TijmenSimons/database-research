...
def get_size_mongo_records(N: int, **k):
    ...
    records = [{key: zlib.compress(str(value).encode('utf-8')) for key, value in r.items()} for r in records]
...
def load_mongo_records(db_name: str, ids: list):
    ...
    for item_id in ids:
        ...
        for item in cursor:
            item = {key: (zlib.decompress(value).decode('utf-8') if key != "_id" else value) for key, value in item.items()}
